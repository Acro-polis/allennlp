...
class CSQADatasetReaderTest(AllenNlpTestCase):
    ...
    def test_reader_reads_multiple_files_direct(self):
        # Read direct questions only from multiple files.
        params = {
            'lazy': True,
            'kg_path':  f'{self.FIXTURES_ROOT}/data/csqa/sample_kg.json',
            'kg_type_path': f'{AllenNlpTestCase.FIXTURES_ROOT}/data/csqa/sample_par_child_dict.p',
        ...
        assert len(instances) == 52

    def test_reader_reads_dpd(self):
        # Read logical forms file.
        params = {
            'lazy': True,
        ...
        dataset = reader.read(qa_path)
        instances = list(dataset)
        assert len(instances) == 0  # TODO this should change when sample_train_action_sequences.p is updated
    ...
    def test_reader_reads_indirect(self):
        ...
        qa_path = f'{self.FIXTURES_ROOT}/data/csqa/sample_qa.json'
        dataset = reader.read(qa_path)
        assert_dataset_correct(dataset, n_instances=24)
